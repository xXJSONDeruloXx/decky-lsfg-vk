...
class ConfigurationService(BaseService):
    ...
    def _generate_script_content(self, config) -> str:
        from .launcher import LauncherGenerator
        return LauncherGenerator(self).generate(config)
    ...
    def update_lsfg_script_from_profile_data(
        self,
        ...
        try:
            ...
            script_content = launcher.generate_profile(profile_data)
            ...
            self.log.info(
                f"Updated lsfg launch script at {self.lsfg_script_path} "
                f"for profile '{profile_data['current_profile']}'"
            )
